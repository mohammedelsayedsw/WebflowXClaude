  ...
  const stats: { delta: string; label: string; detail: string }[] = [
    {
      ...
      detail: "From the 50–60 range typical of PWA storefronts to 90+ on Hyvä – verified per migration.",
    },
    {
      delta: "−65%",
    ...
    },
    {
      ...
      detail: "Native Magento upgrade only. No parallel ScandiPWA compatibility cycle to run, test, and patch.",
    },
    {
      delta: "Days vs weeks",
      label: "Engineer ramp-up",
      detail: "Alpine + Tailwind onboards a Magento dev in days. ScandiPWA conventions take weeks.",
    },
  ];
      ...
      <div className="wrap relative">
        <div className="grid md:grid-cols-[1fr_1.6fr] gap-10 md:gap-16 items-end mb-14 md:mb-20">
          ...
          <Reveal delay={0.1}>
            <p className="text-white/75 text-[15px] md:text-[17px] leading-relaxed max-w-[58ch]">
              Directional outcomes from the ScandiPWA → Hyvä migrations we
        ...
        <Reveal delay={0.2}>
          <p className="mt-12 md:mt-14 max-w-[68ch] text-white/55 text-[12px] md:text-[13px] leading-relaxed">
            ...
            measured against the same store on ScandiPWA. PageSpeed range based
            on Lighthouse mobile audits. We share per-store before/after
