...
export function HowItWorks() {
  ...
  return (
    <section id="how" className="relative z-10 py-28 md:py-36">
      <div className="wrap">
        <Reveal>
          ...
          <div ref={box} className="relative mt-16 md:mt-24">
            {size && (
              <svg
                ...
                fill="none"
              >
                <path d={d} stroke="rgba(255,255,255,0.26)" strokeWidth="1" />
                <g className="motion-reduce:hidden">
                  <circle r="14" fill="rgba(110,247,110,0.16)">
                    <animateMotion dur="16s" repeatCount="indefinite" path={d} />
                  </circle>
                  <circle r="4" fill="var(--sw-mint)">
                    <animateMotion dur="16s" repeatCount="indefinite" path={d} />
                  </circle>
                </g>
              </svg>
            ...
            <ol className="relative grid md:grid-cols-5 gap-y-9 md:gap-y-0 py-[36px] md:py-0 md:px-[84px] md:pb-20">
              {stages.map((s, i) => (
                <li key={s.title} className="relative pl-9 md:pl-0 md:pr-6 md:pt-11">
                  <span
                    aria-hidden
                    className="absolute left-0 top-0 -translate-x-1/2 md:left-[14px] md:-translate-y-1/2 grid h-7 w-7 place-items-center rounded-full border bg-[#05070f] font-head text-[13px] font-semibold tabular-nums"
                    style={{
                      borderColor:
                        i === stages.length - 1 ? "var(--sw-mint)" : "rgba(255,255,255,0.35)",
                      color: i === stages.length - 1 ? "var(--sw-mint)" : "#fff",
                    }}
                  >
                    {i + 1}
                  </span>
                  <h3 className="font-head text-white text-[19px] md:text-[20px] leading-[1.2]">
                    {s.title}
                  </h3>
                  <p className="mt-2 text-white/60 text-[15px] leading-relaxed max-w-[30ch]">
                    {s.line}
                  </p>
                </li>
              ))}
            ...
            <div className="absolute left-1/2 bottom-0 -translate-x-1/2 translate-y-1/2 bg-[#05070f] px-4 whitespace-nowrap">
              <span className="label-code" style={{ color: "var(--sw-mint)" }}>
