"use client";

import { Reveal } from "@/components/primitives/Reveal";

const markets = ["Market 1", "Market 2", "Market 3"];
const channels = ["Email", "Recommendations", "Ads", "Site"];

/** Static layered diagram: markets (bottom) -> engine (middle) -> channels (top). */
function MarketDiagram() {
  return (
    <div
      className="w-full max-w-[440px] mx-auto lg:mr-0 lg:ml-auto rounded-[4px] p-5 sm:p-7"
      style={{ background: "#10132c", border: "1px solid rgba(255,255,255,0.1)" }}
    >
      {/* Top · what customers get */}
      <div className="label-code text-white/45 text-[10px] mb-3 text-center">
        What your customers get
      </div>
      <div className="grid grid-cols-4 gap-1.5">
        {channels.map((c) => (
          <span
            key={c}
            className="font-head flex items-center justify-center whitespace-nowrap rounded-[2px] border border-white/12 bg-white/[0.05] px-1 py-2 text-center text-[8.5px] leading-tight text-white/80"
          >
            {c}
          </span>
        ))}
      </div>

      {/* connector: engine up into the four boxes */}
      <svg
        viewBox="0 0 100 22"
        preserveAspectRatio="none"
        className="my-1.5 h-5 w-full"
        aria-hidden
      >
        {[12.5, 37.5, 62.5, 87.5].map((x, i) => (
          <line
            key={i}
            x1="50"
            y1="22"
            x2={x}
            y2="0"
            stroke="rgba(255,255,255,0.22)"
            strokeWidth="1"
            vectorEffect="non-scaling-stroke"
          />
        ))}
      </svg>

      {/* Middle · engine */}
      <div
        className="rounded-[2px] px-4 py-3.5 text-center"
        style={{
          background: "rgba(110,247,110,0.08)",
          border: "1px solid rgba(110,247,110,0.35)",
        }}
      >
        <div
          className="font-head font-bold text-[14px] leading-tight"
          style={{ color: "var(--sw-mint)" }}
        >
          Multi-Market Personalization Engine
        </div>
        <div className="text-white/60 text-[11px] mt-1.5">
          One view of the customer, markets kept separate
        </div>
      </div>

      {/* connector: three markets up into the engine */}
      <svg
        viewBox="0 0 100 22"
        preserveAspectRatio="none"
        className="my-1.5 h-5 w-full"
        aria-hidden
      >
        {[16.67, 50, 83.33].map((x, i) => (
          <line
            key={i}
            x1={x}
            y1="22"
            x2="50"
            y2="0"
            stroke="rgba(255,255,255,0.22)"
            strokeWidth="1"
            vectorEffect="non-scaling-stroke"
          />
        ))}
      </svg>

      {/* Bottom · markets */}
      <div className="label-code text-white/45 text-[10px] mb-3 text-center">
        Your markets
      </div>
      <div className="grid grid-cols-3 gap-1.5">
        {markets.map((m) => (
          <span
            key={m}
            className="rounded-[2px] border border-white/10 bg-white/[0.03] px-1 py-2.5 text-center leading-tight"
          >
            <span className="font-head block text-[11px] text-white/80">
              {m}
            </span>
            <span className="block text-[9px] text-white/45 mt-1">
              store · ERP · POS
            </span>
          </span>
        ))}
      </div>
    </div>
  );
}

export function InShort() {
  return (
    <section id="in-short" className="bg-lp-bright py-28 md:py-36">
      <div className="wrap">
        <div className="grid gap-12 lg:grid-cols-[1.25fr_1fr] lg:gap-16 lg:items-center">
          {/* LEFT · text */}
          <div className="max-w-[40rem]">
            <Reveal>
              <span className="label-code block text-[var(--sw-black)]/50">
                In short
              </span>
            </Reveal>
            <Reveal delay={0.05}>
              <h2 className="font-head mt-5 max-w-[24ch] text-[26px] leading-[1.08] tracking-[-0.01em] text-[var(--sw-black)] sm:text-[32px] md:text-[40px] lg:text-[46px]">
                What the Multi-Market Personalization Engine is
              </h2>
            </Reveal>
            <Reveal delay={0.1}>
              <p className="mt-7 max-w-[68ch] text-[16px] leading-relaxed text-[var(--sw-black)]/70 md:text-[18px]">
                Today every market runs its own email tool and its own
                recommendation engine. That costs you twice. You pay for the
                same software in every market, and because the customer data
                never joins up, the system does not know what anyone has bought,
                so everyone gets the same generic email.
              </p>
            </Reveal>
            <Reveal delay={0.15}>
              <p className="mt-5 max-w-[68ch] text-[16px] leading-relaxed text-[var(--sw-black)]/70 md:text-[18px]">
                Moving every market onto one platform fixes both. Your team
                works in one place instead of five, and because the data is
                finally joined, the messages can be personal. Each market still
                keeps its own store view, language, and assortment, so nothing
                gets mixed up.
              </p>
            </Reveal>
            <Reveal delay={0.2}>
              <p className="mt-5 max-w-[68ch] text-[16px] leading-relaxed text-[var(--sw-black)]/70 md:text-[18px]">
                Your store, ERP, and POS stay as they are. This sits across them.
              </p>
            </Reveal>
          </div>

          {/* RIGHT · diagram */}
          <Reveal delay={0.15}>
            <MarketDiagram />
          </Reveal>
        </div>
      </div>
    </section>
  );
}
