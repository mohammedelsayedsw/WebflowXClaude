...
const inputs = [
  { name: "ERP", sub: "history · stock · orders" },
  { name: "Retail sales", sub: "history · stock · orders" },
  { name: "Web and media", sub: "history · stock · orders" },
];
const outputs = ["Reorder list", "Size mix", "Seasonality", "Weeks of cover"];

/** Static layered diagram: your data (bottom) -> hub (middle) -> what buyers get (top). */
    ...
    >
      ...
      >
        ...
        <div className="text-white/60 text-[11px] mt-1.5">
          A weekly forecast at the grain you plan in
        </div>
      </div>

      {/* connector: three data sources up into the hub */}
      <svg
        viewBox="0 0 100 22"
        preserveAspectRatio="none"
        className="my-1.5 h-5 w-full"
      ...
      <div className="grid grid-cols-3 gap-1.5">
        {inputs.map((m) => (
          <span
            key={m.name}
            className="rounded-[2px] border border-white/10 bg-white/[0.03] px-1 py-2.5 text-center leading-tight"
          >
            <span className="font-head block text-[11px] text-white/80">
              {m.name}
            </span>
            <span className="block text-[9px] text-white/45 mt-1">{m.sub}</span>
          </span>
        ))}
      </div>
    </div>
