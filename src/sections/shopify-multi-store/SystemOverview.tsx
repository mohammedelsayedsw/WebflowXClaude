"use client";

import { Reveal } from "@/components/primitives/Reveal";

const stores: { name: string; plan: string }[] = [
  { name: "UK Plus", plan: "Shopify Plus" },
  { name: "EU Plus", plan: "Shopify Plus" },
  { name: "US Advanced", plan: "Advanced" },
  { name: "B2B Plus", plan: "Shopify Plus" },
];

const systems: string[] = [
  "apps & app cost",
  "tracking & events",
  "storefront monitoring",
  "custom apps & mobile",
  "CMS / PIM",
  "ERP / OMS / WMS",
  "data & reporting",
  "product feeds",
];

function Plate({
  label,
  footnote,
  children,
}: {
  label: string;
  footnote: string;
  children: React.ReactNode;
}) {
  return (
    <div className="bracket-frame relative rounded-[4px] border border-[var(--sw-black)]/12 bg-white/60 p-6 md:p-10">
      <span className="bracket-bl" />
      <span className="bracket-br" />
      <div className="flex items-center justify-between mb-8">
        <span className="label-code text-[var(--sw-black)]/60">{label}</span>
        <span className="label-code text-[var(--sw-black)]/55">{footnote}</span>
      </div>
      {children}
    </div>
  );
}

export function SystemOverview() {
  return (
    <section id="system" className="bg-lp-bright py-28 md:py-36">
      <div className="wrap">
        <Reveal>
          <div className="label-code text-[var(--sw-black)]/55 mb-5">
            Fig.1 · multi-store operating system
          </div>
          <h2 className="font-head text-[var(--sw-black)] text-[34px] md:text-[48px] lg:text-[56px] leading-[1.05] max-w-[24ch]">
            Multiple Shopify stores,{" "}
            <span className="text-[var(--sw-blue)]">
              one connected operating view
            </span>
          </h2>
          <p className="mt-7 max-w-[64ch] text-[15px] md:text-[17px] leading-relaxed text-[var(--sw-black)]/70">
            Each store keeps its own storefront, theme, and checkout. The
            operating layer sits underneath and connects the systems your stores
            share, so the team sees one picture instead of four.
          </p>
        </Reveal>

        <Reveal delay={0.1}>
          <div className="mt-14 md:mt-20">
            <Plate
              label="Shopify Multi-Store Operating System"
              footnote="Fig.1"
            >
              {/* store row */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                {stores.map((s) => (
                  <div
                    key={s.name}
                    className="rounded-[2px] border border-[var(--sw-black)]/12 bg-white px-4 py-4 text-center"
                  >
                    <div className="font-head text-[var(--sw-black)] text-[16px] md:text-[18px] leading-tight">
                      {s.name}
                    </div>
                    <div className="label-code text-[var(--sw-black)]/60 text-[10px] mt-1.5">
                      {s.plan}
                    </div>
                  </div>
                ))}
              </div>

              {/* converging connector */}
              <div className="relative h-10 md:h-12" aria-hidden>
                <div className="absolute left-1/2 top-0 h-full w-px -translate-x-1/2 bg-[var(--sw-blue)]/30" />
              </div>

              {/* operating bar */}
              <div
                className="rounded-[2px] px-5 py-4 md:py-5 text-center"
                style={{
                  background: "rgba(63, 74, 175, 0.08)",
                  border: "1px solid rgba(63, 74, 175, 0.35)",
                }}
              >
                <div className="label-code text-[var(--sw-blue)]">
                  OperaLayer · operating layer
                </div>
                <div className="font-head text-[var(--sw-black)] text-[15px] md:text-[17px] mt-1.5">
                  one connected view across every store
                </div>
              </div>

              {/* fan-out connector */}
              <div className="relative h-10 md:h-12" aria-hidden>
                <div className="absolute left-1/2 top-0 h-full w-px -translate-x-1/2 bg-[var(--sw-blue)]/30" />
              </div>

              {/* systems row */}
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                {systems.map((sys) => (
                  <div
                    key={sys}
                    className="rounded-[2px] border border-[var(--sw-black)]/10 bg-white/70 px-3 py-3 text-center"
                  >
                    <span className="text-[var(--sw-black)]/75 text-[13px] md:text-[14px] leading-snug">
                      {sys}
                    </span>
                  </div>
                ))}
              </div>

              <div className="mt-8 text-center label-code text-[var(--sw-black)]/55">
                Multiple Shopify stores · one connected operating view
              </div>
            </Plate>
          </div>
        </Reveal>
      </div>
    </section>
  );
}
