        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-8">
          {stats.map((s) => (
            <div key={s.lead} className="flex items-baseline gap-1.5">
              <span className="font-head font-bold text-white text-[14px] md:text-[15px] leading-snug shrink-0">
                {s.lead}
              </span>
              <span className="text-white/45 text-[13px] md:text-[14px] leading-snug">
                &middot; {s.label}
              </span>
            </div>
          ))}
