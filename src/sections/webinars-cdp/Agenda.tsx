        ...
        <ul className="flex flex-col gap-4 md:gap-5">
          {items.map((item, i) => (
            <Reveal key={i} delay={i * 0.07}>
              <li className="rounded-[4px] border border-white/12 bg-white/[0.03] p-7 md:p-8">
                <div className="flex gap-5 md:gap-7">
                  ...
                  <div>
                    <h3
                      className="font-head text-white text-[19px] md:text-[22px] leading-[1.2]"
                      dangerouslySetInnerHTML={{ __html: item.title }}
                    />
                    <p
                      className="mt-3 text-white/75 text-[15px] md:text-[17px] leading-relaxed max-w-[75ch]"
                      dangerouslySetInnerHTML={{ __html: item.body }}
                    />
                    {item.withWho ? (
                      <p className="mt-3 text-white/50 text-[13px] md:text-[14px]">
                        {item.withWho}
                      </p>
                    ) : null}
                  </div>
