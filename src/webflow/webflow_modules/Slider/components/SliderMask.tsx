...
type SliderMaskProps = React.PropsWithChildren<{ className?: string }>;

export type { SliderMaskProps };

const SliderMask = React.forwardRef<HTMLDivElement, SliderMaskProps>(
  function SliderMask({ className = "", children, ...props }, ref) {
    const { setSlideAmount } = React.useContext(SliderContext);
    const [isHovered, setHovered] = React.useState(false);
    const [slides, setSlides] = React.useState<
      React.ReactElement<SliderSlideProps>[]
    >([]);
    const { resumeAutoplay, pauseAutoplay } = useAutoplay();

    React.useEffect(() => {
      const extractNonFragmentChildren = (
        _children: React.ReactNode
      ): React.ReactElement<SliderSlideProps>[] => {
        const childrenList = React.Children.toArray(_children).filter((child) =>
          React.isValidElement(child)
        ) as React.ReactElement<SliderSlideProps>[];

        if (
          childrenList.length === 1 &&
          childrenList[0]?.type === React.Fragment
        ) {
          const fragmentProps = childrenList[0].props as {
